...
**Date:** 2026-05-24
**Companion document:** Section 3 — Test Cases (45 detailed cases)
...
Testing covers three major subsystems:

1. **ML Pipeline** — EfficientNetB0 training, evaluation, ONNX export (offline, on Modal A10G GPU)
2. **Flask Backend** — `/predict`, `/health`, `/classes`, `/lookup` endpoints; ONNX inference; nutrition lookup; OpenRouter insight generation
3. **React Web Dashboard** — landing, login, onboarding, scan upload, history, insights, chatbot, profile
...
| Functional correctness — full 45-case suite | ≥ 90 % pass | **36 % executed Pass at submission; 64 % UAT-pending — zero failures observed** |
| End-to-end scan latency (web, WiFi) | ≤ 5 s | UAT-pending; backend component measured at 1.0–1.6 s |
...
- **Manual UAT** — three personas (Ahmed, Sana, Bilal — see SRS §2.4) walking through every screen on the web app
...
| Integration testing | Backend API end-to-end | `backend/test_api.py` — automated tests | **Pass — run against live Render URL on 2026-05-24** |
| System testing | Web full flows | Manual walkthrough of each TC | UAT-pending |
| Acceptance testing | 3 personas | Manual, scripted | UAT-pending |
| Performance testing | Scan latency, cold-start, RAM footprint | Stopwatch + Render dashboard | Backend warm measured; end-to-end UAT-pending |
| Compatibility testing | Chrome desktop, Safari desktop, Chrome Android (responsive) | Manual smoke tests | UAT-pending |
| Usability testing | First-time user can complete a scan within 3 min | Persona walkthrough timing | UAT-pending |
...
| Mobile web | Android (Chrome) | Responsive smoke test |
...
| Khana 2025 (kashyap077) | South Asian backbone | 156 K images, 236 classes |
...
| 3. System (web) | After every web feature merge | Walk through related TCs | Frontend team |
| 4. UAT — persona walkthroughs | One week before viva | 3 personas × 5 tasks each | Whole team |
| 5. Performance | Once before viva | Stopwatch + Render dashboard | Backend team |
| 6. Pre-viva dry-run | Day before viva | Full demo end-to-end on production URLs | Whole team |
...
The 45 test cases in Section 3 cover all functional requirements (FR-01 through FR-30 in `SRS` §4).

Traceability of FR → TC:

| FR | Requirement | Covered by |
|---|---|---|
| FR-01 — 270-class classifier | TC-API-03, TC-SCAN-04 |
| FR-02 — Top-3 with confidence | TC-API-03 |
| FR-03 — Auto-accept when conf ≥ 0.70 | TC-SCAN-04 |
| FR-04 — Low-conf picker | TC-SCAN-05, TC-SCAN-06, TC-API-06 |
...
| FR-07 — Inference ≤ 3 seconds | TC-PERF-01, TC-API-03 (`processing_time_ms`) |
| FR-08 — Calories / protein / carbs / fat returned | TC-API-03, TC-SCAN-04 |
| FR-09 — Nutrition from local JSON | Verified via `nutrition.py` code review |
| FR-10 — Fallback for missing nutrition | TC-EDGE-01 |
| FR-11 — Non-food state on very low confidence | TC-SCAN-07 |
...
| FR-16 — User correction logging | TC-SCAN-06 |
...
| FR-21 — Onboarding for new users only | TC-ONB-01, TC-ONB-03 |
| FR-22 — Scan saved to DB | TC-SCAN-08 |
| FR-23 — Image uploaded to Storage | TC-SCAN-09 |
| FR-24 — Upload failure non-fatal | TC-SCAN-09 |
| FR-25 — History view | TC-HIST-01 |
| FR-26 — Weekly insights charts | TC-INS-03 |
...
|---|---|---|---|
| NFR-01 — End-to-end scan latency, WiFi | < 5 s | Stopwatch from upload to ResultCard | UAT-pending (backend component 1.0–1.6 s) |
...
- Storage upload failure is non-fatal — TC-SCAN-09
- Network errors surface as user-friendly messages — TC-EDGE-02

### 4.6.4 Security

- **RLS enabled** on `profiles` and `scans` tables — verified in `supabase/schema.sql:48-52` (see TC-EDGE-04)
- **Service-role key never in client code** — verified by `grep -r "SUPABASE_SERVICE" web/` returning zero matches on 2026-05-24
- **`.env` files git-ignored** — verified in `.gitignore`
- Cross-account access test — TC-EDGE-04

### 4.6.5 Usability

- **First scan within 3 minutes** of opening the site — measured during persona walkthroughs (§4.7)
- **No "unhealthy" or "bad" language** in copy — explicitly forbidden in the system prompt at `backend/insights.py:25` ("Never call food unhealthy or bad")
...
|---|---|---|
| Sign up, set goal = muscle_gain | Lands on dashboard with goal saved | UAT-pending |
...
| View "About the Model" | All sections render | UAT-pending |
...
| Open insights after 3 scans | Charts of top dishes render | UAT-pending |
| Re-open the site the next day | Session persisted; lands on dashboard | UAT-pending |
...
Manual review of 5 random insights for tone compliance. Reviewer checks: no "unhealthy / bad / avoid"; warm + culturally appropriate; mentions the user's goal. **To be populated during UAT.**
...
The full ablation results are produced by `model/ablation.py` and stored as `evaluation/ablation_results.csv`. Headline values used in the "About the Model" surface:
...
| Food Scan | 9 | 0 | 3 | 6 | 0 | 0 | 100 % |
| History | 5 | 0 | 0 | 5 | 0 | 0 | — |
| Insights & Chatbot | 6 | 0 | 2 | 4 | 0 | 0 | 100 % |
| Profile | 3 | 0 | 0 | 3 | 0 | 0 | — |
| Backend API | 6 | 6 | 0 | 0 | 0 | 0 | 100 % |
| Performance | 3 | 1 | 0 | 2 | 0 | 0 | 100 % |
| Edge Cases | 4 | 1 | 2 | 1 | 0 | 0 | 100 % |
| **Total** | **45** | **8** | **8** | **29** | **0** | **0** | **100 %** |

**Pass rate on executed test cases: 100 % (16 / 16). Zero failures. Zero blockers.** The 29 UAT-pending rows are scheduled for the pre-viva persona walkthrough (Phase 4 / Phase 6 in §4.4.1).
...
|---|---|---|---|---|---|---|
| BUG-001 | Confirm-pick showed correct label but stale nutrition / insight | P2 | 2026-05-09 | Backend | **Closed** | Fixed — added `GET /lookup?label=X`; the web app fetches fresh nutrition + insight for the chosen label and stores `user_corrected` / `original_top1` |
| BUG-002 | React fragment-without-key warning in `web/src/pages/History.tsx` | P3 | 2026-05-09 | Frontend | Open | Convert `<>` to keyed `<Fragment>` |
| BUG-003 | Web `Today.tsx` didn't show photo preview with ResultCard | P3 | 2026-05-09 | Frontend | **Closed** | Fixed — `previewUrl` (`URL.createObjectURL(file)`) passed down to ResultCard |

**Submission-gate compliance:** 0 P0, 0 P1, 1 P2 (closed), 2 P3 (1 closed, 1 open) — within the documented gate of "zero P0, ≤ 2 P1, P2 / P3 documented".
...
| Demo Wi-Fi unreliable during viva | Medium | Medium | Warm the dyno ~2 minutes before the demo; keep captured screenshots as a backup |
...
| Supervisor | Dr Hassan Khan | __________ | __________ |
